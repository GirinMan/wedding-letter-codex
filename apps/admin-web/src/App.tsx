            ...
            {view === "content" ? (
              <Panel title="기본 콘텐츠" description="이름, 인사말, 일정과 장소를 관리합니다." actions={<button className="button button--primary" onClick={() => void saveContent()}>{saving ? "저장 중…" : "변경사항 저장"}</button>}>
                ...
                <div className="field-grid">
                  ...
                  <Field label={`지도 확대 ${invitation.draftContent.event.map.zoom}`}><input type="range" min="6" max="21" value={invitation.draftContent.event.map.zoom} onChange={(event) => updateContent((draft) => { draft.event.map.zoom = Number(event.target.value); })} /></Field>
