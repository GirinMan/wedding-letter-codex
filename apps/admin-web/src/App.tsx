            ...
            {view === "content" ? (
              <Panel title="기본 콘텐츠" description="이름, 인사말, 일정과 장소를 관리합니다." actions={<button className="button button--primary" onClick={() => void saveContent()}>{saving ? "저장 중…" : "변경사항 저장"}</button>}>
                ...
                <div className="field-grid">
                  ...
                  <Field label="네이버 지도 Client ID" wide><input placeholder="NCP Dynamic Map ncpKeyId" value={invitation.draftContent.event.map.naverMapClientId} onChange={(event) => updateContent((draft) => { draft.event.map.naverMapClientId = event.target.value; })} /></Field>
                  <Field label={`네이버 지도 확대 ${invitation.draftContent.event.map.zoom}`}><input type="range" min="6" max="21" value={invitation.draftContent.event.map.zoom} onChange={(event) => updateContent((draft) => { draft.event.map.zoom = Number(event.target.value); })} /></Field>
                  <Field label="네이버지도 길찾기 링크" wide><input type="url" placeholder="선택: 앱/웹 길찾기 URL" value={invitation.draftContent.event.map.navigation.naverUrl} onChange={(event) => updateContent((draft) => { draft.event.map.navigation.naverUrl = event.target.value; })} /></Field>
                  <Field label="티맵 길찾기 링크" wide><input type="url" placeholder="선택: 티맵 공유 URL" value={invitation.draftContent.event.map.navigation.tmapUrl} onChange={(event) => updateContent((draft) => { draft.event.map.navigation.tmapUrl = event.target.value; })} /></Field>
                  <Field label="카카오내비 길찾기 링크" wide><input type="url" placeholder="선택: 카카오내비 공유 URL" value={invitation.draftContent.event.map.navigation.kakaoNaviUrl} onChange={(event) => updateContent((draft) => { draft.event.map.navigation.kakaoNaviUrl = event.target.value; })} /></Field>
