...
5. 새 방문객 사진은 기본 승인 상태로 즉시 공개된다. 관리자는 사후 거절로
   비공개 처리하거나 다시 승인할 수 있다. 삭제·복구 기능도 유지한다.
6. `/photos` 또는 `/<invitation-slug>/photos`에서 별도 사진 앨범을 연다.
   청첩장의 “함께한 사진 보기” 링크에서도 이동할 수 있다. 같은 업로드 DB와
   객체 저장소를 사용하며, 승인된 사진만 최신순으로 조회한다. 30장씩 더 보기,
   사진 확대, 원본 열기를 지원하고 삭제·미승인 사진은 제외한다.
   사진 업로드는 기존 청첩장에서 설정된 시작 시각 이후에만 가능하다.
   앨범과 청첩장의 “사진 올리기”는 같은 모달을 열며, 한 번에 최대 100장을
   선택해 사진당 15MB까지 올릴 수 있다. 사진별 진행·실패 상태를 표시하고
   실패한 사진과 아직 보내지 않은 사진만 이어서 올린다. 메모는 업로드 묶음에
   한 번 입력하며 재시도에도 같은 이름·메모를 유지한다.
   한 장씩 기존 API로 전송해 저장·승인 정책을 유지하며 업로드 rate limit은
   IP당 시간당 600건이다. 한 장씩 전송해 전체 파일을 동시에 메모리에 읽지
   않는다. 연결 오류·요청 제한·서버 오류가 나면 남은 전송을 멈춘다.
   사용자가 멈추면 현재 사진을 마친 뒤 정지한다. 모달을 닫아도 현재 페이지
   안에서는 전송이 계속되지만, 새로고침·브라우저 종료 후 자동 복구는 지원하지
   않는다. 전송 중 페이지를 떠날 때는 브라우저 경고를 요청하며 모바일 화면
   잠금으로 전송이 중단될 수 있음을 안내한다.
   앨범의 이름·일정·표지 사진·업로드 설명과 디자인은 발행된 청첩장에서
   가져온다. 업로드 시작 전/후 안내가 자동 전환되며, 확대 화면에서 이전·다음
   사진으로 이동할 수 있다. 표지는 업로드 영역의 기본 사진을 우선 사용하고,
   없으면 청첩장 대표 사진을 사용한다. 모바일에서는 작은 표지와 접이식 안내로
   상단을 압축하고, 업로드 시각을 청첩장 시간대 기준 1시간씩 묶어 표시한다.
   페이지를 더 불러오면 같은 시간대 묶음에 사진을 이어 붙인다. 그 안에서
   같은 이름·메모가 연속된 사진을 묶고 구분선 아래 이름과 메모를 표시한다.
   이름·메모가 모두 없으면 사진만 표시한다.

앨범은 30장씩 표시하고 다음 한 페이지의 목록 정보만 미리 조회한다.
첫 9장의 썸네일은 데이터 로드 직후, 나머지는 화면에서 800px 이내로 가까워지면
동시 3개씩 비동기로 요청·디코딩한다. 목록은 최대 480px WebP 썸네일
(`?size=thumbnail`), 확대는 1280px 이미지를 사용한다. 업로드 시작 이후에는
초 단위 타이머를 멈추고, 이미 표시한 사진 타일은 불필요하게 다시 렌더링하지 않는다.
청첩장 갤러리도 동시 3개씩 이미지를 미리 불러온다. 공개 이미지의 `?size=display`는 최대 1280px WebP를
객체 저장소에 캐시하며 원본 URL과 원본 파일은 보존한다. 공개·승인·삭제 조건을
확인한 후 변환본을 제공하고, HEIC 등 변환을 지원하지 않는 형식은 원본으로
돌아간다. 처음 보는 사진의 변환·네트워크 시간까지 없앨 수는 없다.
